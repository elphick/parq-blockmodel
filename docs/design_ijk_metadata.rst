...
- The payload includes a schema version and may include optional
  ...
  ``global_id_encoding`` is optional and used only when ``global_id``
...
8. Terminology: block_id, global_id, and ijk
--------------------------------------------
...
**global_id**
    ...
    - Generated by ``encode_global_coordinates`` using quantization + offset
    - **Not a spatial index** – should not be used for range queries

The distinction is important:

- Use **ijk** for logical block queries and geometric operations
- Use **global_id** for stable cross-model references and external joins
...
- **global_id** is a stable 64-bit position key, useful for cross-model joins
- Geometry metadata is compact, versioned, and stored in Parquet key-value pairs

Decode ``global_id`` from ``DataFrame.attrs``
--------------------------------------------

Consumers that only have a DataFrame (no ``ParquetBlockModel`` instance)
can still decode ``global_id`` values using the attached metadata payload:
